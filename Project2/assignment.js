...
/* ── Profile bridge (shared localStorage key) ── */
...
/**
 * Apply stored name + avatar to every nav slot:
 *   • small circle avatar  (.avatar-wrap > img)
 *   • popup header avatar  (.pp-header > img)
 *   • popup name           (.pp-name)
 */
function applyProfileToNav() {
    ...
    if (p.avatar) {
        const navImg   = document.querySelector('.avatar-wrap > img');
        const popupImg = document.querySelector('.pp-header > img');
        if (navImg)   navImg.src   = p.avatar;
        if (popupImg) popupImg.src = p.avatar;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ► Apply stored profile to nav avatar + popup name
    applyProfileToNav();
