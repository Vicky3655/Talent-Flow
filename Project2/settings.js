/* ═══════════════════════════════════════════
   ...
   settings.js
   Reads profile from localStorage ("tf_student_profile")
   on load; patches it back when the user saves.
═══════════════════════════════════════════ */
...
// ── Populate settings UI from saved profile ──

function loadProfileIntoSettings() {
    const p = getProfile();
    if (!Object.keys(p).length) return; // nothing saved yet
    ...
    if (nameEl  && p.fullName) nameEl.value  = p.fullName;
    if (emailEl && p.email)    emailEl.value = p.email;
    if (bioEl   && p.bio)      bioEl.value   = p.bio;

    // Display name heading
    const displayNameEl = document.getElementById('profileDisplayName');
    if (displayNameEl && p.fullName) displayNameEl.textContent = p.fullName;

    // Profile photo — large and nav avatar
    if (p.avatar) {
        const profileImgEl = document.getElementById('profileImg');
        if (profileImgEl) profileImgEl.src = p.avatar;

        const navAvatarEl = document.querySelector('.avatar img');
        if (navAvatarEl) navAvatarEl.src = p.avatar;
    }
}
...
avatarInput.addEventListener('change', () => {
    ...
    reader.onload = e => {
        ...
        // Persist immediately so dashboard picks it up
        patchProfile({ avatar: dataURL });
    };
...
document.getElementById('saveProfileBtn').addEventListener('click', () => {
    ...
    // Persist to localStorage so profile page + dashboard reflect the change
    patchProfile({ fullName: name, email, bio });
