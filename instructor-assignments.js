...
function loadAssignments() {
    try {
        const saved = localStorage.getItem('tf_instructor_assignments');
        return saved ? JSON.parse(saved) : deepClone(SEED_ASSIGNMENTS);
    } catch { return deepClone(SEED_ASSIGNMENTS); }
}

function save() {
    localStorage.setItem('tf_instructor_assignments', JSON.stringify(assignments));
}
...
document.getElementById('createForm').addEventListener('submit', e => {
    ...
    if (editingId) {
        ...
        if (a) {
            ...
            a.course       = fd.get('course');
            a.instructions = fd.get('instructions');
            a.dueDate      = fd.get('dueDate');
            a.maxScore     = parseInt(fd.get('maxScore')) || 100;
            a.assignTo     = fd.get('assignTo');
...
function addManualSubmission(aId, sId) {
